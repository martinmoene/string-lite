...
table = (
    ...
    , ( 'CMakeLists.txt'
        , r'set\W+string_bare_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( string_bare_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+string_bare_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( string_bare_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\string_bare_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'string_bare_version = "{major}.{minor}.{patch}"\n' )
    ...
    , ( 'include/nonstd/string.hpp'
        , r'\#define\s+string_bare_MAJOR\s+[0-9]+\s*$'
        , '#define string_bare_MAJOR  {major}' )

    , ( 'include/nonstd/string.hpp'
        , r'\#define\s+string_bare_MINOR\s+[0-9]+\s*$'
        , '#define string_bare_MINOR  {minor}' )

    , ( 'include/nonstd/string.hpp'
        , r'\#define\s+string_bare_PATCH\s+[0-9]+\s*$'
        , '#define string_bare_PATCH  {patch}\n' )
)
